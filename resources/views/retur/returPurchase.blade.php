<x-layout>
  <x-header :href="route('purchases.show', $purchase->id)" title="Retur Pembelian #{{ $purchase->id }}" />
  <div class="mx-auto max-w-screen-xl">
    ...
    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
      <form id="returnForm" method="POST" action="{{ route('returs.store.purchase') }}">
        @csrf
          ...
          <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
              <tr>
                ...
                <th class="px-6 py-3">Qty Beli</th>
                <th class="px-6 py-3">Qty Retur</th>
              ...
              <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <td class="px-4 py-3 text-center">
                  @if ($remainingQty <= 0)
                    ...
                    @else
                    <input type="number" name="items[{{ $item->id }}][return_quantity]" min="0" max="{{ $remainingQty }}" value="0" class="returQty w-20 text-center text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded px-2 py-1">
                    @endif
                </td>
                ...
                @if ($remainingQty <= 0)
                  ...
                  @else
                  <td class="px-4 py-3">
                    <input type="text" name="items[{{ $item->id }}][note]" class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded px-2 py-1" placeholder="Opsional">
                  </td>
                  @endif

                  <td class="px-4 py-3 text-right">
                    Rp {{ number_format($item->buyPrice) }}
                  </td>
                  <td class="px-4 py-3 text-right subtotal">Rp 0</td>

                  <!-- Hidden inputs -->
                  <input type="hidden" name="items[{{ $item->id }}][product_id]" value="{{ $item->product_id }}">
                  <input type="hidden" name="items[{{ $item->id }}][price]" value="{{ $item->buyPrice }}">
              </tr>
              @endforeach
            </tbody>
          </table>
