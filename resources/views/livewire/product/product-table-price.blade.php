            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="px-4 py-3" wire:click="setSortBy('sellPrice')">Harga</th>
                        <th scope="col" class="px-4 py-3" wire:click="setSortBy('effectiveDate')">Tangal Berlaku</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($productPrices as $index => $product)
                    <tr wire:key="{{ $product->id }}" class="border-b dark:border-gray-700">
                        <th scope="row" class="px-4 py-3">{{ $productPrices->firstItem() + $index }}</th>
                        <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ number_format($product->sellPrice, 0, ',', '.') }}</td>
