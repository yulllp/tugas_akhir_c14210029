            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    ...
                    <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-7 flex-shrink-0">
                        <a type="button" href="{{ route('suppliers.create') }}" class="flex items-center justify-center text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3" wire:click="setSortBy('created_at')">Bergabung</th>
                        ...
                        <tbody>
                            ...
                            <tr wire:key="{{ $supplier->id }}" class="border-b dark:border-gray-700">
                                <th scope="row" class="px-4 py-3">{{ $suppliers->firstItem() + $index }}</th>
                                <td class="px-4 py-3">{{ $supplier->id }}</td>
                                <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $supplier->name }}</td>
                                <td class="px-4 py-3">{{ $supplier->address }}</td>
                                <td class="px-4 py-3">{{ $supplier->phone }}</td>
                                <td class="px-4 py-3">{{ $supplier->created_at }}</td>
                                <td class="px-4 py-3 flex items-center justify-start space-x-3">
                                    <a href="{{ route('suppliers.edit', $supplier->id) }}"
                                        class="text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                        Edit
                                    </a>
                                    <form action="{{ route('suppliers.destroy', $supplier->id) }}" method="POST" onsubmit="return confirm('Apakah anda yakin akan menghapus pelanggan ini?');">
                                        @csrf
                                        @method('DELETE')
