<x-layout>
  <x-header :href="route('suppliers.index')" title="{{ $title }}"></x-header>
  <div class="mx-auto max-w-screen-xl gap-8 grid sm:grid-cols-2 grid-cols-1 mt-4">
    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden col-span-1">
      <form class="p-4 md:p-5" action="{{ route('suppliers.update', $supplier) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="grid gap-6 mb-6 grid-cols-1">
          <div class="col-span-1">
            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama</label>
            <input type="text" value="{{ $supplier->name }}" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Masukan nama" required="">
            @error('name')
          ...
          <div class="col-span-1">
            ...
            <input type="text" value="{{ $supplier->address }}" name="address" id="address" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Alamat" required="">
            @error('address')
            <p class="text-red-500 text-sm">{{ $message }}</p>
            @enderror
          </div>
          <div class="col-span-1">
            ...
            <input type="text" value="{{ $supplier->phone }}" name="phone" id="phone" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="No. Telepon" required="">
            @error('phone')
            <p class="text-red-500 text-sm">{{ $message }}</p>
            @enderror
          </div>
    ...
    <div class="col-span-1 flex max-h-42 p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400" role="alert">
      ...
      <div>
        <span class="font-medium">Pastikan syarat pembuatan supplier ini terpenuhi:</span>
        <ul class="mt-1.5 list-disc list-inside">
          <li>Nama supplier tidak boleh sama</li>
          <li>Periksa kembali daftar nama supplier agar menghindari pembuatn data supplier yang berlebihan</li>
